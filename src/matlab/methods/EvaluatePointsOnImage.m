...
figure(1);
imshow(rescale(squeeze(i1(:,:,200))));
lineColorMap = GetLineColorMap('custom', cell(2,1)); %'custom-hsv'
...
for j = 1:imgN
    ...
    for i = 1:curvesN
        k = k + 1; 
        if imgN ~= 1 
        	colors(k) = lineColorMap(key{j});
        else 
            colors{k} = lineColorMap(key{i});
        end 
    end 
end
...
if isNormalized
    ...
    Plots(2, @PlotSpectra, curves, names, 420:730, 'Reflectance', 'Normalized Measurements', colors);
else 
    SetSetting('plotName', fullfile(basedir, 'sampleCurves_norm.jpn'));
    Plots(2, @PlotSpectra, curves, names, 380:780, 'Measurement', 'Raw Measurements', colors);
end
